...
* 2. [定义数组时，声明的变量，是数组的第一个元素的存储地址，](#-1)
* 3. [数组的优点：](#-1)
	* 3.1. [1. 查询性能好，指定查询的某个位置](#-1)
* 4. [数组的缺点：](#-1)
	* 4.1. [1. 数组空间是连续的](#-1)
	* 4.2. [2. 数组定长](#-1)
* 5. [chrome v8引擎 对数组进行了优化](#chromev8)
* 6. [插入排序](#-1)
* 7. [选择排序](#-1)
* 8. [冒泡排序](#-1)
* 9. [归并排序](#-1)
...
* 28. [二叉平衡搜索树](#-1)
* 29. [还原二叉树](#-1)
	* 29.1. [给出前序中序还原二叉树，写出后序遍历](#-1)
	* 29.2. [给出后序中序还原二叉树，写出前序遍历](#-1)
* 30. [图的创建](#-1)
* 31. [最小生成树](#-1)
* 32. [图的广度优先搜索](#-1)
* 33. [图的深度优先遍历](#-1)
...
# 数组
##  1. <a name=''></a>数组定义时，确定数组的长度（数组定长）
js数组扩容需要消耗CPU，影响性能（操作系统分配内存空间）
js的数组在底层是一个链表（哈希表），添加元素时，可以自动扩容
##  2. <a name='-1'></a>定义数组时，声明的变量，是数组的第一个元素的存储地址，
...
```
##  3. <a name='-1'></a>数组的优点：
###  3.1. <a name='-1'></a>1. 查询性能好，指定查询的某个位置
##  4. <a name='-1'></a>数组的缺点：
###  4.1. <a name='-1'></a>1. 数组空间是连续的
如果数组比较大时，当系统的空间碎片较多的时候，容易存不下
###  4.2. <a name='-1'></a>2. 数组定长
数组内容很难进行添加和删除操作

##  5. <a name='chromev8'></a>chrome v8引擎 对数组进行了优化
...
###  14.1. <a name='-1'></a>基于有序序列和二分查找，是对二分查找的优化
遵循以下公式：
...
b.right = e;
```
###  23.1. <a name='-1'></a>递归遍历

####  23.1.1. <a name='-1'></a>前序遍历
...
###  23.2. <a name='-1'></a>非递归实现遍历

####  23.2.1. <a name='-1'></a>前序遍历
```js
function func1(root) {
    if (root === null) return [];
    const nodeArr = [];
...
##  26. <a name='-1'></a>二叉树的搜索

###  26.1. <a name='-1'></a>深度优先搜索
...
###  29.1. <a name='-1'></a>给出前序中序还原二叉树，写出后序遍历
```js
...
###  29.2. <a name='-1'></a>给出后序中序还原二叉树，写出前序遍历
